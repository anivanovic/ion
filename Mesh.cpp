...
Mesh::Mesh(Vertex* vertices, unsigned int numVertices, unsigned int* indices,
           unsigned int numIndices, GLenum mode) {
  ...
  glBufferData(GL_ARRAY_BUFFER, numVertices * sizeof(texCoor[0]), &texCoor[0],
               GL_STATIC_DRAW);
  glEnableVertexAttribArray(1);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_VBO[INDEX_VB]);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, numIndices * sizeof(indices[0]),
